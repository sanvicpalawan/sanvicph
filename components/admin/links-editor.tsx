...
import { PLACE_LINK_ICONS, placeLinkIcon } from "@/lib/place-links";
...
// each with a label, a URL and an icon chosen from the curated Lucide set.
    ...
    <section className="links-editor inline-media-picker">
      <div className="inline-media-title">
        <div>
          ...
          <h3>Links</h3>
          <span>Repeatable links shown as tappable pills near the top of the public page — Booking.com, Agoda, Facebook, website, “book direct”… Reorder with the arrows, then save the location. Empty when nothing is added.</span>
        </div>
      ...
      {links.map((link, index) => {
        ...
        return (
          <article key={link.id} className={`link-editor-row ${pickerOpen ? "open" : ""}`}>
            ...
            {pickerOpen && (
              <div className="link-icon-grid" aria-label={`Icon for ${link.label || `link ${index + 1}`}`}>
                {PLACE_LINK_ICONS.map((option) => (
                  <button key={option.name} type="button" className={link.icon === option.name ? "active" : ""} aria-pressed={link.icon === option.name} aria-label={option.name} title={option.name} onClick={() => patch(link.id, { icon: option.name })}>
                    <option.Icon/>
                  </button>
                ))}
              </div>
