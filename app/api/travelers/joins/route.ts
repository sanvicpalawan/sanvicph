...
export async function POST(request: Request) {
  try {
    ...
    const { data: item } = await supabaseAdmin().from("content_items").select("id").eq("kind", "opportunity").eq("slug", id).eq("status", "published").maybeSingle();
    if (!item) return json({ error: "This opportunity is no longer available." }, 404);
